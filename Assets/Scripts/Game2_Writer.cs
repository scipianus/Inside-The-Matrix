...
public class Game2_Writer {
	...
	public static void refreshPathsFile(int dimmension) {
        refreshFile(pathsFilePath + dimmension);
	}
	...
	public static void appendPathsFile(int pathScore, List<Pair<int, int>> path, int dimmension) {
        string line = pathScore.ToString();
        for (int i = 0; i < path.Count; ++i) {
            line += " " + path[i].fst + " " + path[i].snd;
        }
        appenToFile(pathsFilePath + dimmension, line);
	}
}
